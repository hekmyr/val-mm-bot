...
class MockUser:
    """Mock Discord User for testing/development purposes"""
    ...
    async def send(self, message: str):
        """Mock sending a DM to the user"""
        log(f"[MOCK] Sending DM to user {self.id}: {message}")
        return None


class MockReady:
    """Auto-mark mock users as ready for testing"""
    
    @staticmethod
    def auto_ready_mock_users(bestof: int):
        """Auto-mark all test users as ready"""
...
class MockTeamBalancer:
    """Mock team balancer for testing"""
